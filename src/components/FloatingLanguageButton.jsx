    ...
    <div className={`fixed right-4 top-1/2 transform -translate-y-1/2 z-50 ${className}`}>
      <div className="flex items-center">
              ...
              <div className="flex flex-col px-1 pt-2 space-y-2">
                {LANGUAGES.map(lang => (
                  <button
                    ...
                    className={`flex items-center space-x-3 px-3 py-2 rounded-md hover:bg-white/60 focus:outline-none transition-all duration-150 ${lang.code === current.code ? 'bg-teal-600 text-white shadow-md' : 'text-gray-700'}`}
                  >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="w-12 h-12 rounded-full bg-teal-600 hover:bg-teal-700 text-white shadow-lg flex items-center justify-center text-xl ml-2"
